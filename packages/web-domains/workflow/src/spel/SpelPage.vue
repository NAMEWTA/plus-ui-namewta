...
import type { SpelForm, SpelQuery, SpelVO } from '@namewta/domain-workflow';
...
const { sys_show_hide, sys_normal_disable } = runtime.dicts('sys_show_hide', 'sys_normal_disable');
...
const getList = async () => {
  await withLoading(async () => {
    const res = await runtime.service.listSpel(queryParams.value);
...
const handleUpdate = async (row?: Partial<SpelVO>) => {
  ...
  const res = await runtime.service.getSpel(spelId);
  ...
  spelFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      await withButtonLoading(async () => {
        if (form.value.id) {
          await runtime.service.updateSpel(form.value);
        } else {
          await runtime.service.addSpel(form.value);
        }
      });
...
const handleDelete = async (row?: Partial<SpelVO>) => {
  ...
  await runtime.confirm('是否确认删除流程spel表达式定义编号为"' + spelIds + '"的数据项？');
  await runtime.service.deleteSpel(spelIds);
