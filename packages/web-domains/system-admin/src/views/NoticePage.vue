...
import type { NoticeForm, NoticeQuery, NoticeVO } from '@namewta/domain-system-admin';
...
const {
  ...
  delete: delNotice,
  get: getNotice,
  attachmentUrls: getNoticeAttachmentDownloadUrls,
  list: listNotice,
  update: updateNotice
} = runtime.service.resources.notices;
const modal = { confirm: runtime.confirm, msgSuccess: runtime.success };
const { replaceOssContentUrls, sanitizeHtml } = runtime;
const { sys_notice_status, sys_notice_type } = runtime.dicts('sys_notice_status', 'sys_notice_type');
...
const openDetail = async (noticeId: string | number) => {
  ...
  data.noticeContent = await replaceOssContentUrls(data.noticeContent, urls);
