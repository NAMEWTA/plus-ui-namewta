...
- 默认远端基线：`https://github.com/NAMEWTA/plus-ui-namewta`，分支 `main`。引用远端文件时必须同时写明 `branch=main` 和仓库内路径。
...
- 不要把本机绝对路径写进 skill 输出或参考文档；跨环境参考统一使用 GitHub 仓库 URL、分支和仓库内相对路径。
...
- `gen` 是 plus-ui 前端项目内的生成模板；默认远端位置是 `https://github.com/NAMEWTA/plus-ui-namewta/tree/main/gen`，分支必须标记为 `main`。
