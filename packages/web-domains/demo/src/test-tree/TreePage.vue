...
import type { TreeForm, TreeQuery, TreeVO } from '@namewta/domain-demo';
...
const { runtime } = defineProps<{ runtime: DemoWebRuntime }>();
...
const getList = async () => {
  await withLoading(async () => {
    const res = await runtime.service.listTree(queryParams.value);
    const data = buildTree<TreeVO>(res.data ?? [], 'id', 'parentId');
    if (data) {
      treeList.value = data;
    }
  });
...
const getTreeselect = async () => {
  const res = await runtime.service.listTree();
  treeOptions.value = [];
  const data: TreeOption = { id: 0, treeName: '顶级节点', children: [] };
  data.children = buildTree<TreeOption>(res.data ?? [], 'id', 'parentId');
...
const handleUpdate = async (row: Partial<TreeVO>) => {
  ...
  const res = await runtime.service.getTree(row.id as string | number);
...
const submitForm = () => {
  treeFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await runtime.service.updateTree(form.value).finally(() => (buttonLoading.value = false));
      } else {
        await runtime.service.addTree(form.value).finally(() => (buttonLoading.value = false));
      }
...
const handleDelete = async (row: Partial<TreeVO>) => {
  await runtime.confirm('是否确认删除测试树编号为"' + row.id + '"的数据项？');
  setLoading(true);
  await runtime.service.deleteTree(row.id as string | number).finally(() => setLoading(false));
  await getList();
