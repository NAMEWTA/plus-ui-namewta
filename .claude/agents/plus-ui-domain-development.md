...
2. 用源码确认真实入口；当前唯一激活的 App 组合在 `apps/admin-web/src/router/adminManifestRegistry.ts`，其他 App 均为 README-only 占位。
...
6. 后端是最终授权者；菜单投影使用 `@namewta/platform-app-runtime`，Vue 权限指令使用 `@namewta/web-kit-permission`，App 只拥有导航状态与组合，所有失败路径必须关闭。
7. Client Web、移动 Web、小程序和 Taro 适配器在独立规格激活前保持 README-only。
