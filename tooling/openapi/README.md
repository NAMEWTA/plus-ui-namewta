...
- `openapi:fetch -- --source <url-or-file> --backend-commit <40-character-sha>` validates OpenAPI 3.0/3.1 input, persists snapshot and provenance in an immutable digest revision, then atomically activates that revision through `current.json`.
