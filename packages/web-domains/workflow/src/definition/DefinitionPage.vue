...
import type {
  ...
  FlowDefinitionForm,
  FlowDefinitionQuery,
  FlowDefinitionVO as FlowDefinitionVo
...
const getTreeselect = async () => {
  const res = await runtime.service.categoryTree();
...
const getList = async () => {
  await withLoading(async () => {
    const resp = await runtime.service.listDefinitions(queryParams.value);
...
const getUnPublishList = async () => {
  await withLoading(async () => {
    const resp = await runtime.service.listUnpublishedDefinitions(queryParams.value);
    processDefinitionList.value = resp.data?.rows;
    total.value = resp.data?.total;
  });
};
...
const handleDelete = async (row?: Partial<FlowDefinitionVo>) => {
  ...
  await runtime.confirm('是否确认删除流程定义编码为【' + defList + '】的数据项？');
  setLoading(true);
  await runtime.service.deleteDefinition(id).finally(() => setLoading(false));
  await handleQuery();
...
const handlePublish = async (row?: Partial<FlowDefinitionVo>) => {
  ...
  setLoading(true);
  await runtime.service.publishDefinition(row.id).finally(() => setLoading(false));
  activeName.value = '0';
  await handleQuery();
...
const handleProcessDefState = async (row: Partial<FlowDefinitionVo>, status: number | string | boolean) => {
  ...
  try {
    setLoading(true);
    await runtime.confirm(msg);
    await runtime.service.setDefinitionActive(row.id, !!status);
    await handleQuery();
...
const handlerBeforeUpload = () => {
  if (selectCategory.value === 'ALL') {
    runtime.error('顶级节点不可作为分类！');
    return false;
  }
  if (!selectCategory.value) {
    runtime.error('请选择左侧要上传的分类！');
    return false;
  }
};
//部署文件
...
const handleUpdate = async (row?: Partial<FlowDefinitionVo>) => {
  ...
  const res = await runtime.service.getDefinition(id);
...
const handleSubmit = async () => {
  defFormRef.value.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await runtime.service.updateDefinition(form.value).finally(() => setLoading(false));
      } else {
        await runtime.service.addDefinition(form.value).finally(() => setLoading(false));
        activeName.value = '1';
      }
      runtime.success('操作成功');
      closeModelDialog();
...
const handleExportDef = () => {
  runtime.download(`/workflow/definition/exportDef/${ids.value[0]}`, {}, `${flowCodeList.value[0]}.json`);
};
</script>
