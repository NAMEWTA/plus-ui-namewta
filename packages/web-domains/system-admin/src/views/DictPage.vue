...
const {
  ...
  add: addData,
  update: updateData
} = runtime.service.resources.dictData;
const {
  list: listType,
  get: getType,
  delete: delType,
  ...
  update: updateType,
  refreshCache
} = runtime.service.resources.dictTypes;
...
const typeDialog = reactive<DialogState>({
  visible: false,
  title: ''
});
...
const dataDialog = reactive<DialogState>({
  visible: false,
  title: ''
});
...
const submitDataForm = () => {
  dataFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      runtime.dictCache.remove(dataQueryParams.value.dictType);
      modal.msgSuccess('操作成功');
      dataDialog.visible = false;
      await getDataList();
    }
