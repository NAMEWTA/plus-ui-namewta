<template>
  <div class="p-2 app-container monitor-notify-page">
    ...
    <DetailDrawer ref="detailDrawerRef" :runtime="runtime" />
  </div>
</template>

<script setup name="NotifyMonitor" lang="ts">
import type { NotifyListVO, NotifyQuery } from '@namewta/domain-operations';
import { onMounted, reactive, ref } from 'vue';
import type { OperationsWebRuntime } from '../runtime';
import { useLoading, useSearchToggle, useTableSelection } from '../composables';
import { parseTime } from '../utils';
import DetailDrawer from './notify/NotificationDetailDrawer.vue';

const { runtime } = defineProps<{ runtime: OperationsWebRuntime }>();
const { list: listNotify, delete: delNotify, clean: cleanNotify } = runtime.service.notifications;
...
const handleDelete = async (row?: Partial<NotifyListVO>) => {
  ...
  await runtime.confirm(`是否确认删除通知日志 ${notifyLogIds}？`);
...
const handleClean = async () => {
  await runtime.confirm('是否确认清空全部通知监控数据？');
  await runtime.confirm('清空后完整正文、目标和投递明细均无法恢复，是否继续？');
  await cleanNotify();
  await getList();
  runtime.success('清空成功');
};
...
@use '../styles/page-shell' as pageShell;
