...
import type { LoginInfoQuery, LoginInfoVO } from '@namewta/domain-system/monitor';
...
const { runtime } = defineProps<{ runtime: MonitorWebRuntime }>();
const { list, delete: delLoginInfo, clean: cleanLoginInfo, unlock: unlockLoginInfo } = runtime.service.loginInfo;

const { sys_device_type } = toRefs<any>(runtime.dicts('sys_device_type'));
const { sys_common_status } = toRefs<any>(runtime.dicts('sys_common_status'));
...
const handleDelete = async (row?: LoginInfoVO) => {
  ...
  await runtime.confirm('是否确认删除访问编号为"' + infoIds + '"的数据项?');
...
const handleClean = async () => {
  await runtime.confirm('是否确认清空所有登录日志数据项?');
...
const handleUnlock = async () => {
  ...
  await runtime.confirm('是否确认解锁用户"' + username + '"数据项?');
  await unlockLoginInfo(username);
  runtime.success('用户' + username + '解锁成功');
};
/** 导出按钮操作 */
const handleExport = () => {
  runtime.download(
