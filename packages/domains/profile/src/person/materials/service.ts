...
export interface PersonMaterialService {
  accessUrl(
...
export function createPersonMaterialService(http: HttpClient): PersonMaterialService {
  ...
  const ownerPath = (ownerType: MaterialOwnerType, ownerId: Identifier) =>
    `/profile/person/materials/${segment(ownerType)}/${segment(ownerId)}`;
  return Object.freeze<PersonMaterialService>({
