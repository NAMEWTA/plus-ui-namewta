...
description: 前端总入口。用于 plus-ui 前端项目中的标准 CRUD 页面、新增 API/types、复杂列表页增强、树筛选、导入导出、权限按钮与弹窗表单等任务，并根据任务类型选择合适的前端子 agent；默认基线为 GitHub 仓库 NAMEWTA/plus-ui-namewta 的 main 分支。
---
...
- 冲突时优先相信当前项目真实页面，其次是公共组件和工具，再其次才是本仓库 `main` 分支的 `gen` 模板。
