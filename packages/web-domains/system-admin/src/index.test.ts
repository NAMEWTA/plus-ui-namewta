...
describe('system-admin web manifest', () => {
  it('publishes all server-facing governance keys and permission groups', () => {
    const manifest = createSystemAdminWebDomain(runtime);
  ...
  it('registers only for an explicitly selected App composition', () => {
    ...
    const admin = composeAppRuntime({
      appId: 'admin-web',
      domainModules: [systemAdminDomainModule],
      manifests: [manifest],
      selectedDomainIds: ['system-admin'],
      selectedManifestIds: ['web-domain-system-admin']
    });
