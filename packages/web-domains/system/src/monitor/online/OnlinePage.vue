...
import type { OnlineQuery, OnlineVO } from '@namewta/domain-system/monitor';
import { onMounted, ref, toRefs } from 'vue';
import type { MonitorWebRuntime } from '../runtime';
import { parseTime } from '../utils';

const { runtime } = defineProps<{ runtime: MonitorWebRuntime }>();
const { list: initData, forceLogout } = runtime.service.online;
...
const handleForceLogout = async (row: Partial<OnlineVO>) => {
  try {
    await runtime.confirm('是否确认强退名称为"' + row.userName + '"的用户?');
  } catch {
    return;
  }
