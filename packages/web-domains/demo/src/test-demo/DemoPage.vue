...
import type { DemoForm, DemoQuery, DemoVO } from '@namewta/domain-demo';
...
const getList = async () => {
  await withLoading(async () => {
    const res = await runtime.service.listDemo(queryParams.value);
...
const handleUpdate = async (row?: Partial<DemoVO>) => {
  ...
  const res = await runtime.service.getDemo(demoId);
...
const submitForm = () => {
  demoFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await runtime.service.updateDemo(form.value).finally(() => (buttonLoading.value = false));
      } else {
        await runtime.service.addDemo(form.value).finally(() => (buttonLoading.value = false));
      }
      runtime.success('修改成功');
      closeDialog();
      await getList();
    }
  });
...
const handleDelete = async (row?: Partial<DemoVO>) => {
  ...
  await runtime.confirm('是否确认删除测试单编号为"' + demoIds + '"的数据项？');
  await runtime.service.deleteDemo(demoIds);
