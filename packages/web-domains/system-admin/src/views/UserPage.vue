    ...
    >
      <el-form ref="userFormRef" :model="form" :rules="rules" label-width="80px">
        <el-row>
          ...
          <el-col :span="12" v-if="form.userId == null || form.userId != currentUserId()">
        ...
        <el-row v-if="form.userId == null || form.userId != currentUserId()">
          <el-col :span="24">
...
const optionselect = runtime.service.posts.options;
const listRole = runtime.service.roles.list;
const users = runtime.service.users;
const api = {
  listUser: users.list,
  getUser: users.get,
  addUser: users.add,
  ...
  resetUserPwd: users.resetPassword,
  changeUserStatus: users.changeStatus,
  unlockUser: users.unlock,
  deptTreeSelect: users.departmentTree
};
...
const handleRoleClientChange = async (clientId?: string | number) => {
  ...
  try {
    ...
    if (requestId !== roleClientRequestId) return;
  ...
  userFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      if (form.value.userId) {
        ...
        if (form.value.userId == currentUserId()) {
