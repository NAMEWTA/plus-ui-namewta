<template>
  <div class="p-2 app-container monitor-operlog-page">
    ...
    <!-- 操作日志详细 -->
    <OperInfoDialog ref="operInfoDialogRef" :runtime="runtime" />
  </div>
</template>

<script setup name="Operlog" lang="ts">
import type { OperLogForm, OperLogQuery, OperLogVO } from '@namewta/domain-operations';
...
import OperInfoDialog from './operlog/OperationInfoDialog.vue';

const { runtime } = defineProps<{ runtime: OperationsWebRuntime }>();
const { list, delete: delOperlog, clean: cleanOperlog } = runtime.service.operationLogs;

const { sys_oper_type, sys_common_status, sys_device_type } = toRefs<any>(
  runtime.dicts('sys_oper_type', 'sys_common_status', 'sys_device_type')
);
...
const handleDelete = async (row?: Partial<OperLogVO>) => {
  ...
  await runtime.confirm('是否确认删除日志编号为"' + operIds + '"的数据项?');
...
const handleClean = async () => {
  await runtime.confirm('是否确认清空所有操作日志数据项?');
  await cleanOperlog();
  await getList();
