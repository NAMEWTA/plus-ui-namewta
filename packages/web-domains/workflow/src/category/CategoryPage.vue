...
import type { CategoryForm, CategoryQuery, CategoryVO } from '@namewta/domain-workflow';
import type { FormInstance as ElFormInstance, FormRules, TableInstance as ElTableInstance } from 'element-plus';
import { computed, nextTick, onMounted, reactive, ref, toRefs } from 'vue';
import type { WorkflowWebRuntime } from '../runtime';
import { useFormDialog, useLoading, useSearchReset, useSearchToggle, useTreeTableExpand } from '../composables';
...
const getList = async () => {
  await withLoading(async () => {
    const res = await runtime.service.listCategories(queryParams.value);
...
const getTreeselect = async () => {
  const res = await runtime.service.listCategories();
...
const handleUpdate = async (row: Partial<CategoryVO>) => {
  ...
  const res = await runtime.service.getCategory(row.categoryId);
  ...
  categoryFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.categoryId) {
        await runtime.service.updateCategory(form.value).finally(() => (buttonLoading.value = false));
      } else {
        await runtime.service.addCategory(form.value).finally(() => (buttonLoading.value = false));
      }
...
const handleDelete = async (row: Partial<CategoryVO>) => {
  await runtime.confirm('是否确认删除"' + row.categoryName + '"的分类？');
  setLoading(true);
  await runtime.service.deleteCategory(row.categoryId).finally(() => setLoading(false));
  await getList();
