<template>
  <div class="enterprise-review-page" v-loading="loading">
    <template v-if="context">
      <header>
        <div>
          <h2>企业认证审核</h2>
          <span>法定代表人信息属于企业快照，申请账户是拟绑定的认证负责人。</span>
        </div>
        <el-tag>{{ context.status }}</el-tag>
      </header>
      <el-descriptions :column="2" border>
        <el-descriptions-item label="申请编号">{{ context.applicationId }}</el-descriptions-item>
        <el-descriptions-item label="申请账户">{{ context.applicantUserId }}</el-descriptions-item>
        <el-descriptions-item label="提交序号">{{ context.submissionSeq }}</el-descriptions-item>
        <el-descriptions-item label="提交时间">{{ context.submittedTime }}</el-descriptions-item>
      </el-descriptions>
      <section>
        <h3>不可变企业申请快照</h3>
        <pre>{{ snapshot }}</pre>
...
import { safeEnterpriseErrorMessage } from './logic';
...
async function load() {
  ...
  loading.value = true;
  try {
    context.value = (await runtime.service.enterprise.archive.review(applicationId.value)).data;
  } catch (error) {
...
async function download(materialRefId: Identifier) {
  try {
    const access = (await runtime.service.enterprise.archive.reviewMaterial(applicationId.value, materialRefId)).data;
...
async function completeReview() {
  ...
  await runtime.confirm('确认提交当前企业流程审核结果？');
  saving.value = true;
  try {
...
async function overrideReview() {
  ...
  saving.value = true;
  try {
    await runtime.service.enterprise.archive.decide(applicationId.value, {
